...
class PathFinderBot(PythonBot):
    ...
    def do_turn(self) -> None:
        ...
        strongest_owned_tile = max(
            self.game.map.own_tiles, key=lambda x: x.strength)
        ...
        path = Algorithms.aStar(
            self.game.map, strongest_owned_tile.tile, target)
...
if __name__ == "__main__":
    ...
    redis_config_file = os.path.dirname(
        __file__) + './config.json'  # get file path
    config: dict = json.load(open(redis_config_file))[
        'redisConfig']  # read file into dict
    redis_config = dict(
        host=config['HOST'],
        port=config['PORT'],
